...
public:
  ...
  Iterator find(const Key_type& k) const {
    ...
    // //Iterator find(const Key_type& k) const {
         return variable.find({k, Value_type()});
      }
 ...
 Value_type& operator[](const Key_type& k) {
  Iterator it = variable.find({k, Value_type()});
  if (it == end()) {

  variable.insert({k, Value_type()});
    return variable.find({k, Value_type()})->second;
...
std::pair<Iterator, bool> insert(const Pair_type& val) {
    ...
    if (variable.find(val)!= end()){
          return {variable.find(val),false};
      
    }
    variable.insert(val);

    return {variable.find(val),true};
...
BinarySearchTree<Pair_type, PairComp> variable;
};
